...
func (middleware *AuthMiddleware) ServeHTTP(writer http.ResponseWriter, request *http.Request) {
	...
	if request.URL.Path == "/api/v1/login" || request.URL.Path == "/api/v1/register" || request.URL.Path == "/api/v1/users" || request.URL.Path == "/api/v1/users/forgot-password" || request.URL.Path == "/api/v1/users/set-new-password" || request.URL.Path == "/api/v1/otp/validate" || request.URL.Path == "/api/v1/otp/send" || request.URL.Path == "/api/v1/users/change-password" || request.URL.Path == "/api/v1/users/update" {
